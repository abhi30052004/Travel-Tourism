...
1. New Tables
- `subscriptions` - Stores newsletter subscriber information
  - `id` (uuid, primary key)
  - `name` (text)
  - `email` (text, unique)
  - `created_at` (timestamp)
- `rss_feeds` - Stores RSS feed configurations for AI Content Agent
  - `id` (uuid, primary key)
  - `name` (text)
  - `url` (text)
  - `category` (text)
  ...
  - `created_at` (timestamp)
- `rss_items` - Stores fetched RSS content
  - `id` (uuid, primary key)
  - `title` (text)
  - `source` (text)
  - `category` (text)
...
- Allow anon + authenticated read/write for single-tenant public data.
*/
